...
int print_char(char c, int col, int row, char attr) {
    ...
    //check if it meet the boundaries of the screen and scroll up
    if (offset >= MAX_ROWS * MAX_COLS * 2) {
        int i;
        for (i = 1; i < MAX_ROWS; i++){
            //"move" it row by row
            zzylib_memcpy(((char*)(get_offset(0, i) + VIDEO_ADDRESS)), ((char*)(get_offset(0, i - 1) + VIDEO_ADDRESS)), MAX_COLS * 2);
        }
        //blank the last line
...
void vga_print_backspace() {
    ...
    print_char(0x08, col, row, COLOR_WHITEONBLACK);
}